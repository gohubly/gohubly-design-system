	...
	return (
		<css.Container padding={padding} fluid={fluid}>
          ...
          helperText
            ...
            {emails.length <= 0 ? 'Adicionar e-mail' : 'Adicionar mais e-mails'}
					</Button>
				</Box>
			)}

			{canAddMultipleEmails && selectedMedia === EShareMedia.email && !success && emails.length > 0 && emails.map(email => (
				<css.EmailsContainer>
					...
					<css.EmailContainer title={email}>
