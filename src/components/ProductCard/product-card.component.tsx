...
export const ProductCard: React.FC<iProductCard> = ({
  ...
  const [selectedItem, setSelectedItem] = useState<typeof skus[0]>(
    skus.at(0) as typeof skus[0]
  );

  const bestPrice =
    selectedItem.bestPrice === 0
    ...
    <ProductItem>
      ...
      {hasSkuSelector && (
        <Select
          ...
          onSelect={({ value }): void =>
            setSelectedItem(
              skus.find(({ id }) => id === value.toString()) as typeof skus[0]
            )
          }
      ...
      {!hideButton && (
        <Button size="SM" hierarchy="primary" fluid {...buttonProps}>
