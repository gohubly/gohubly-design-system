...
export const Input: React.FC<iInput> = React.forwardRef(
  (
    ...
    useEffect(() => {
      if (innerRef?.current) {
        if (props.prefix) {
          const prefix = innerRef?.current?.previousSibling as HTMLElement;
          const { width, padding } = window.getComputedStyle(prefix);

          innerRef.current.style.setProperty("--prefix-width", width);
          innerRef.current.style.setProperty(
            "--prefix-padding",
            padding.replace("0px ", "")
          );

          innerRef.current.classList.add("input-with-prefix");
        }

        if (props.suffix) {
          const suffix = innerRef?.current?.nextSibling as HTMLElement;
          const { width, padding } = window.getComputedStyle(suffix);

          innerRef.current.style.setProperty("--suffix-width", width);
          innerRef.current.style.setProperty(
            "--suffix-padding",
            padding.replace("0px ", "")
          );

          innerRef.current.classList.add("input-with-suffix");
        }
      }
    }, [innerRef, props.prefix, props.suffix]);
